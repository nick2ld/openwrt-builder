...
2. В блоке `Роутеры` начните вводить модель, например `GL-MT6000`, `Archer C7` или `OpenWrt One`.
3. Выберите найденное устройство.
4. Сервис сам заполнит `target`, `subtarget`, `profile`, `arch` и полный список пакетов прошивки.
5. При необходимости поправьте список пакетов в карточке роутера.
